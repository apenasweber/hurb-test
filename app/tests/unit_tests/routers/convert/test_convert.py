...
@pytest.mark.parametrize(
    "from_this, to, amount, converted_value, status_code",
    [
        ("BRL", "EUR", 10, 1.97, 200),
        ("HURB", "TEST", 2, 0.2, 200),
...
def test_should_convert_currency_from_database(
    ...
    convert_response = client.get(
        f"/convert/?from_this={from_this}&to={to}&amount={amount}"
    )
    ...
    assert convert_response_data["from_this"] == from_this
    assert convert_response_data["to"] == to
    assert convert_response_data["amount"] == amount


@pytest.mark.parametrize(
    "from_this, to, amount, status_code, missing_currency",
    [
        ("HURB", "EUR", 2, 404, "HURB"),
...
def test_should_not_find_currency_in_database(
    client: TestClient, from_this, to, amount, status_code, missing_currency
):
